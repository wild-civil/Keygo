...
## 9. 蓝牙 Appearance 图标（讨论稿，未实施）

> 状态：**仅讨论，未动代码**（2026-07-18）。目标：让系统蓝牙列表里的设备图标随双模式变化（汽车 / 电瓶车），但需先用 nrf-connect 真机确认手机到底认哪个 Appearance 值。
> 关联固件点：`peripheral.c:71-73`（当前广播写死 `GAP_APPEARE_GENERIC_WATCH`=0x00C0）、`APP/include/appearance.h:218-234`（已备车辆类宏）。

### 9.1 现状

- 手机蓝牙列表显示的图标**由广播里的 GAP Appearance（AD Type 0x19）决定，与 HID 服务无关**。
- 当前 `peripheral.c` 写死 `GAP_APPEARE_GENERIC_WATCH`，所以列表显示手表。
- `appearance.h` 已备好车辆类：`GAP_APPEARE_GENERIC_VEHICLE`(0x08C0)、`GAP_APPEARE_VEHICLE_CAR`(0x08C1，汽车)、`GAP_APPEARE_VEHICLE_2_WHEEL`(0x08C3，两轮)、`GAP_APPEARE_VEHICLE_MOTORBIKE`(0x08C4)、`GAP_APPEARE_VEHICLE_SCOOTER`(0x08C5)、`GAP_APPEARE_VEHICLE_MOPED`(0x08C6，电瓶车)。

### 9.2 方案（双模式动态 Appearance）

| 模式 | 候选 Appearance | 值 |
|---|---|---|
| 汽车 car | `GAP_APPEARE_VEHICLE_CAR` | `0x08C1` |
| 电瓶车 ebike | `GAP_APPEARE_VEHICLE_2_WHEEL` / `GAP_APPEARE_VEHICLE_MOPED` | `0x08C3` / `0x08C6` |

做法：启动/切 MODE 时读 `MODE_ADDR`，重建 `advertData` 中 `GAP_ADTYPE_APPEARANCE` 两字节，调 `GAPRole_SetParameter(GAPROLE_ADVERT_DATA, ...)` + 重启广播即可。`peripheral.c` 顶部需 `#include "appearance.h"`。

### 9.3 关键澄清（与协议无关）

- **Appearance 是蓝牙 SIG《Assigned Numbers》定义的 16 位展示枚举**（GAP 服务 0x1800 / 特征 0x2A01 / 广播 AD 0x19），**纯展示元数据，无任何关联 GATT 服务或协议**。
- 设备可声明 `car(0x08C1)` 却仍只用 HID + 自定义 FF 服务，手机照连，appearance 值不参与握手/门控。
- 手机画出某图标 = ROM 内置「值 → 图标资源」查表，厂商自选收录哪些值。汽车数字钥匙是热门场景，很多 Android ROM 收录了 `0x08C1`；电瓶车/滑板车类冷门，多数 ROM 可能回退默认蓝牙图标（功能零影响）。
- **影响范围**：Appearance 只影响系统蓝牙列表 / 配对弹窗图标；不影响 App 内 UI（App 图标自绘）；iOS 基本不为这类 appearance 画彩色图标。

### 9.4 待验证（nrf-connect 真机测试计划）

手机蓝牙列表里**确实出现过汽车图标**，但尚未确认是哪一个 Appearance 值触发的，且 `0x08C1 / 0x08C3 / 0x08C6` 三个候选均未在 KeyGo 上实测：

1. 用 **nrf-connect** 连接一个「手机列表显示为汽车图标」的真实蓝牙设备，读取其广播包 / GAP 服务 `0x2A01`，确认手机到底把哪个 Appearance 值映射成了汽车图标（验证是否就是 `0x08C1`，还是车辆大类 `0x08C0` 等）。
2. 固件改为候选值后，用 nrf-connect + 手机系统列表交叉验证：car 是否真显汽车、ebike 候选值是否显图标（还是回退默认）。
3. 视真机结果再定最终取值（若 `0x08C1` 经验证即汽车，则按 §9.2 落地；若实为准其他值，以实测为准）。

### 9.5 决策门槛

- 落地固件改动前，必须先完成 §9.4 的 nrf-connect 验证，避免凭推断改了却显不出预期图标。
- 即便电瓶车类显不出专属图标，也**不影响任何功能**，仅为列表装饰差异。
