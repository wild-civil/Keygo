...
| | 极速模式 | 舒适模式 | 手动模式 |
|:--|:--|:--|:--|
| **恢复延迟** | 走到车边已就绪 | 亮屏后 2-8 秒 | 完全手动（点按钮） |
| **额外功耗** | ≈ 0.2%/天 | ≈ 0.01%/天 | 0 |
| **额外权限** | 后台定位 | 无 | 无 |
| **检测原理** | 加速度计→缓存GPS→条件扫描 + GPS围栏 + 心跳兜底 | 亮屏→延迟2s→扫描8s→连接 | 无（完全手动） |
| **后台轮询** | 无（加速度计 SoC 级，~0.5mA） | 无（完全事件驱动） | 无 |
| **续航影响** | 极小（传感器常开 + 条件扫描） | 几乎为零（仅亮屏时触发） | 零 |
| **实现复杂度** | 高 | 低 | 低（已有基础） |
| **适合人群** | 每日通勤，追求无感体验 | 大部分用户 ★默认 | 露营/贴身等需完全托管 |
...
- 手动模式下**不自动连接**：`_shouldAutoReconnect()` 直接返回 false，onShow / 亮屏 / 围栏 / 心跳 / 原生后台扫描全部不触发自动连
- 手动模式下**不自动锁车**：连接后向固件下发 `autolock=0`，固件 RSSI 状态机跳过自动解锁/上锁，只响应手动 `UNLOCK`/`LOCK` 命令
- 用户全程通过界面按钮手动控制连接与锁车
...
### 15.2.1 ★ v3.24 更新：三模式调整为「舒适 / 手动 / 极速」，新增手动模式
- **动机**：露营等"人一直贴着车"场景，RSSI 在阈值附近抖动会让自动逻辑反复触发——
  ① APP 反复自动连接/重连；② 固件按 RSSI 阈值反复自动解锁/上锁。
- **手动模式（替换原省电模式）**：`autoReconnectMode = 'manual'`
  - APP 侧：`_shouldAutoReconnect()` 闸门直接拦截（onShow/亮屏/围栏/心跳/原生后台扫描全不触发）；
    `_ensureForegroundService()` 不启动保活；切换模式时停止前台服务。
  - 固件侧：连接后下发 `autolock=0`，`KeyGo_ProcessStateMachine()` 跳过 RSSI 自动锁判定，
    只响应手动 `UNLOCK`/`LOCK` 命令（FF02 状态通知新增 `al` 字段上报使能状态）。
  - 兼容：固件默认 `g_cfgAutoLockEnable=1`（旧 App 无此字段则维持自动锁）；纯文本协议向后兼容。
- **注意**：手动模式彻底关闭自动行为，适合需要完全托管的贴身场景。
- **★ 回归防护（v3.24-fix1）**：APP 侧 `utils/ble.js` 的 `sendConfig()` 白名单**必须包含 `autolock` 字段**，
  否则 `_syncConfigToDevice()` 下发的 `autolock=0` 会被静默丢弃，固件 `g_cfgAutoLockEnable` 始终为默认 `1`，
  RSSI 阈值自动解锁仍会触发（即"手动模式仍自动解锁"的 bug）。验证：手动模式连接后 FF02 状态通知 `"al":0`。
- **★ 回归防护（v3.24-fix2）**：APP 侧 `stores/ble.js` 的 `_restoreConfig()` 模式恢复列表**必须包含 `'manual'`**，
  否则手动模式持久化值 `'manual'` 在启动时恢复失败、回落默认 `comfort`（极速/舒适正常、手动回退的 bug）。
  旧值 `power_saver` 需迁移为 `manual`。
- **★ 回归防护（v3.24-fix3）**：配置页 `pages/config/config.vue` 解析 FF02 `al` 字段写入 store 时，**必须存数值 `0/1` 而非布尔**。
  错误写法 `this.autoLockEnabled = data.al === 1` 会得到布尔 `true/false`，而 UI 判断用的是 `=== 0 / === 1`，
  两者永远对不上 → 手动模式（固件已正确回报 `al:0`）被误显为"已开启 / 未生效"（v3.24-fixb 已踩坑）。
  正确写法：`this.autoLockEnabled = (Number(data.al) === 0) ? 0 : 1`。
  验证：手动模式连接后，配置页"设备自动锁"应显示「已关闭」而非「已开启」。
