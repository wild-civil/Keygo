# KeyGo v3.32.2 实现状态与待办核对清单

> 用途：记录 v3.32.2 已落地能力 + 未实现/待核对项，便于后期逐一核对。
> 草拟：2026-07-13（帮助页优化时同步整理）
> 关联：帮助页 `app/BLE_Key_Go_App/pages/help/help.vue` 的「规划中功能」卡片仅列摘要，本文件为完整台账。
...
| 版本号 | KEYGO_FW_VERSION 同步为 `3.32.2`，帮助页标注同步为 v3.32.2 | 2026-07-13 软回滚自 v3.33 |
...
| T5 | 固件版本号上报 | FF02 `v` 已 bump 为 `3.32.2`（与帮助页一致），需重烧生效 | 重烧后 App 控制台 fwVersion=3.32.2 |
...
- [ ] T3 配置持久化（见②修复）
- [ ] T4 App 自动回推
- [x] T5 固件版本号 bump（3.32.2，2026-07-13，待重烧）
- [ ] T6~T11 见上
