...
基于 **uni-app (Vue 3)** 开发，用于配合 KeyGo 设备固件。

## 功能

| 功能 | 说明 |
|------|------|
| 🔍 **BLE 扫描连接** | 搜索附近 KeyGo 设备并连接 |
| 📶 **RSSI 实时显示** | 通过 Notify 接收设备上报的 RSSI 信号强度 |
| 🔓 **手动解锁** | 通过 BLE 发送解锁命令到 KeyGo 设备 |
| 🔒 **手动锁车** | 通过 BLE 发送锁车命令到 KeyGo 设备 |
| 🚗 **后备箱控制** | 通过 BLE 发送后备箱命令到 KeyGo 设备 |
...
1. KeyGo 设备上电，固件开始 BLE 广播
2. 打开 App → 自动扫描 KeyGo 设备
