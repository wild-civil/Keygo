/*
 * BLE-Key-Go v3.4 — BLE 安全增强 + 广播名修复
 *
 * ── v3.4 核心变更 ──────────────────────────────────────
 *   ● BLE 广播名截断修复
 *     UUID 移到 Scan Response，广播包显示完整 "KeyGo-71C65A"
 *   ● 加密状态精准追踪
 *     新增 encryptionEstablished 标记，区分"GATT 连接"与"配对加密"
 *     Status JSON "enc" 字段从此真实反映加密状态
 *   ● PIN 修改 + 自动重启
 *     修改 PIN 后 ESP.restart() 确保 bond 数据彻底清除
 *   ● 所有连接/断连/认证回调重置 encryptionEstablished
 *
 * ── v3.3 核心变更 ──────────────────────────────────────
...
void setup() {
    ...
    Serial.println();
    Serial.println("============================================");
    Serial.println("  BLE-Key-Go v3.4 starting...");
    ...
    Serial.println();
    Serial.printf("==== BLE-Key-Go v3.4 | Device: %s ====\n", deviceName);
#if BLE_KEY_GO_STACK_BLUEDROID
...
void notifyStatus() {
    ...
    // dn=deviceName d2=customName sn=serialNumber(MAC后缀)
