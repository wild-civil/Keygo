/*
 * BLE-Key-Go v3.3 — 设备身份认证（BLE Bonding）
 *
 * ── v3.3 核心变更 ──────────────────────────────────────
...
public:
    ...
    void handleChangePIN(String params) {
        ...
        // ★ v3.3: 删除所有 bond + 断开连接 + 保存新 PIN
        deleteAllBonds();    // ★ v3.3: 清 NVS bond + 主动断连
        pairingPIN = newPIN;
        pinDefault = (pairingPIN == DEFAULT_PAIRING_PIN);  // ★ v3.3: 改回出厂 PIN 时恢复标记
        savePIN();                                          // ★ v3.3: 独立刷入 NVS
...
void setup() {
    ...
    Serial.println();
    Serial.println("============================================");
    Serial.println("  BLE-Key-Go v3.3 starting...");
    ...
    Serial.println();
    Serial.printf("==== BLE-Key-Go v3.3 | Device: %s ====\n", deviceName);
#if BLE_KEY_GO_STACK_BLUEDROID
