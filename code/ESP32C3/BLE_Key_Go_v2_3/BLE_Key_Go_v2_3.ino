...
// ★ v2.4: 主人分享密钥（Share Key）
//   主人生成一个临时密钥分享给他人，客人输入后可跳过物理按键+PIN 直接接管设备。
//   密钥为 RAM-only（不持久化），设备重启后自动失效。
#define SHARE_KEY_LEN       8                        // 分享密钥长度（字符）
#define SHARE_KEY_MAX_HOURS 168                      // 最大有效期 7 天
String  shareKey = "";                               // 当前有效分享密钥（空=未启用）
unsigned long shareKeyGeneratedAt = 0;               // millis 生成时刻
unsigned long shareKeyDurationMs = 86400000UL;       // 有效期（默认 24h）
bool    shareVerified = false;                       // 当前连接通过分享密钥认证
bool    shareKeyJustGenerated = false;               // 标记：下次 notifyStatus 时输出密钥明文
...
uint8_t  nimblePeerAddr[6] = {0};                     // ★ 保存远端地址用于 BIND
#endif
...
class ServerCallbacks : public BLEServerCallbacks {
    ...
    void onDisconnect(BLEServer* server, esp_ble_gatts_cb_param_t* param) override {
        ...
        shareVerified = false;                       // ★ v2.4: 清除分享密钥认证
        latestRSSI = -999;
...
public:  // ★ 供 serial 命令和 FF03 BLE 命令共同调用
    void onWrite(BLECharacteristic* characteristic) override {
        ...
        // ★ v2.4: 分享密钥认证（无需鉴权，客人端输入密钥）
        if (upper.startsWith("AUTH:")) {
            handleShareAuth(cmd.substring(5));
            return;
        }
        ...
        // ★ v2.4: 分享密钥管理（需鉴权，主人专用）
        if (upper.startsWith("SHARE:GEN:")) {
            handleShareGenerate(cmd.substring(10));
            return;
        }
        if (upper == "SHARE:REVOKE") {
            handleShareRevoke();
            return;
        }
    ...
    void handleBindCommand() {
        ...
        // ★ v2.4: 三个免 PIN 验证通道：
        //   1. 物理按键配对模式 → 已通过物理接触授权
        //   2. PIN 已验证通过
        //   3. 分享密钥已验证通过
        if (!pairingModeActive && !pinVerified && !shareVerified) {
    ...
    void saveConnectedMac() {
        ...
        newMac = macToString(nimblePeerAddr, 6);
#endif
    ...
    // ★ v2.4: 生成分享密钥（主人端调用，需鉴权）
    //   格式: SHARE:GEN:HH  (HH=有效期小时数，1-168，默认24)
    void handleShareGenerate(String hoursStr) {
        hoursStr.trim();
        int hours = hoursStr.toInt();
        if (hours < 1 || hours > SHARE_KEY_MAX_HOURS) {
            hours = 24;
        }

        // 生成 8 位十六进制随机密钥
        char buf[SHARE_KEY_LEN + 1];
        for (int i = 0; i < SHARE_KEY_LEN; i++) {
            uint8_t nibble = esp_random() & 0x0F;
            buf[i] = "0123456789ABCDEF"[nibble];
        }
        buf[SHARE_KEY_LEN] = '\0';
        shareKey = String(buf);
        shareKeyGeneratedAt = millis();
        shareKeyDurationMs = hours * 3600000UL;
        shareKeyJustGenerated = true;  // 下次 notifyStatus 时输出密钥明文

        Serial.printf("[SHARE] key generated: %s, expires in %d hours\n",
                      shareKey.c_str(), hours);
        notifyStatus();  // 立即推送包含 share_key 的 JSON
    }

    // ★ v2.4: 撤销分享密钥（主人端调用，需鉴权）
    void handleShareRevoke() {
        if (shareKey.length() == 0) {
            Serial.println("[SHARE] no active share key to revoke");
            return;
        }
        Serial.printf("[SHARE] key revoked: %s\n", shareKey.c_str());
        shareKey = "";
        shareKeyGeneratedAt = 0;
        shareKeyJustGenerated = false;
        shareVerified = false;  // 撤销后任何人无法再用旧密钥
        notifyStatus();
    }

    // ★ v2.4: 验证分享密钥（客人端调用，无需鉴权）
    //   格式: AUTH:XXXXXXXX
    void handleShareAuth(String enteredKey) {
        enteredKey.trim();
        enteredKey.toUpperCase();

        // 检查是否有有效分享密钥
        if (shareKey.length() == 0) {
            Serial.println("[SHARE] AUTH failed: no active share key");
            shareVerified = false;
            notifyStatus();
            return;
        }
        ...
        // 检查是否过期
        if (millis() - shareKeyGeneratedAt >= shareKeyDurationMs) {
            Serial.println("[SHARE] AUTH failed: share key expired");
            shareKey = "";  // 自动清理过期密钥
            shareKeyGeneratedAt = 0;
            shareVerified = false;
            notifyStatus();
        ...
        // 比对密钥
        if (enteredKey == shareKey) {
            shareVerified = true;
            connectionAuthorized = true;
            Serial.println("[SHARE] AUTH success: guest authorized via share key");
        } else {
            shareVerified = false;
            Serial.printf("[SHARE] AUTH failed: entered '%s', expected '%s'\n",
                          enteredKey.c_str(), shareKey.c_str());
        }
        notifyStatus();
    }
};
...
void setup() {
    ...
    Serial.println();
    Serial.println("============================================");
    Serial.println("  BLE-Key-Go v2.4 starting...");
    ...
    if (bondedMac.length() > 0) {
        ...
        Serial.println("[SECURITY] NOT BOUND — PIN verification required (default: 123456)");
...
void loop() {
    ...
    // ★ 安全：断开未授权连接（30秒宽限期供用户输入 PIN 并绑定）
    {
        ...
        if (deviceConnected && !connectionAuthorized) {
            ...
            if (millis() - unauthConnectedAt > 30000) {
                Serial.println("[SECURITY] unauthorized device timeout (30s) — disconnecting");
    ...
    if (pairingModeActive && millis() - pairingModeStartMs >= PAIRING_MODE_TIMEOUT_MS) {
        ...
        // ★ v2.3: 配对模式结束后重新评估授权
        //   已绑定设备（MAC 匹配）→ 保持授权；未绑定设备 → 取消授权，需走 PIN+BIND 流程
        if (deviceConnected && bondedMac.length() == 0) {
            connectionAuthorized = false;
            Serial.println("[SECURITY] deauthorized — device not bound, PIN+BIND required");
            // 不立即断开，走上方的 30s 宽限期逻辑
        }
    }

    // ★ v2.4: 分享密钥过期检测
    if (shareKey.length() > 0 && millis() - shareKeyGeneratedAt >= shareKeyDurationMs) {
        Serial.println("[SHARE] share key expired, auto-revoked");
        shareKey = "";
        shareKeyGeneratedAt = 0;
        shareKeyJustGenerated = false;
        shareVerified = false;
...
/*
 * ★ 判断远端 MAC 是否被允许执行控制命令
 *
 * 放行条件（满足其一即可）：
 *   1. 配对模式激活中（pin 9 按键触发 30s 窗口） → 接受（已物理接触）
 *   2. 远端 MAC 与 NVS 存储的 bondedMac 完全匹配 → 接受
 *
 *   ★ v2.3 安全策略：未绑定设备不再自动授权。
 *      未绑定设备可连接但不能控制，必须通过 PIN 验证 + BIND 后才能解锁操作。
 *      PIN / BIND 命令本身不需要鉴权，所以绑定流程不受影响。
 */
bool isMacAuthorized(const uint8_t* mac, int len) {
    ...
    // ★ 通道 3: 分享密钥已在本次连接中验证通过
    //   注意：shareVerified 在 onConnect 之前为 false，需客人端连接后发送 AUTH: 命令
    //   此处的检查对 onConnect 时无效（shareVerified 尚未设为 true），
    //   但客人端连接后 30s 宽限期内发送 AUTH: 通过后，connectionAuthorized 会被设为 true
    if (shareVerified && shareKey.length() > 0 &&
        millis() - shareKeyGeneratedAt < shareKeyDurationMs) {
        return true;
    }

    // ★ 未绑定设备不允许自动授权，必须走 PIN → BIND 流程
    if (bondedMac.length() == 0) {
        return false;
    }
...
void handleSerialCommand() {
    ...
    if (upper == "HELP") {
        ...
        pairingModeStartMs = millis();
        Serial.printf("[SECURITY] pairing mode active for %d seconds\n", PAIRING_MODE_TIMEOUT_MS / 1000);
    } else if (upper.startsWith("SHARE:GEN:")) {
        // ★ v2.4: 串口生成分享密钥
        if (g_commandCallbacks) {
            g_commandCallbacks->handleShareGenerate(cmd.substring(10));
        }
    } else if (upper == "SHARE:REVOKE") {
        // ★ v2.4: 串口撤销分享密钥
        if (g_commandCallbacks) {
            g_commandCallbacks->handleShareRevoke();
        }
    } else if (upper.startsWith("AUTH ")) {
        // ★ v2.4: 串口验证分享密钥: "auth ABCD1234"
        if (g_commandCallbacks) {
            g_commandCallbacks->handleShareAuth(cmd.substring(5));
        }
...
void notifyStatus() {
    ...
    payload += ",\"share_active\":";                     // ★ v2.4: 是否有有效分享密钥
    payload += String((shareKey.length() > 0 && millis() - shareKeyGeneratedAt < shareKeyDurationMs) ? 1 : 0);
    payload += ",\"share_verified\":";                   // ★ v2.4: 当前连接是否通过分享密钥认证
    payload += String(shareVerified ? 1 : 0);
    if (shareKeyJustGenerated) {
        // 仅在新生成密钥时返回明文（一次性），用后即清除标记
        payload += ",\"share_key\":\"";
        payload += shareKey;
        payload += "\"";
        shareKeyJustGenerated = false;
    }
    payload += ",\"share_expiry\":";                     // ★ v2.4: 剩余有效秒数
    if (shareKey.length() > 0) {
        unsigned long remain = shareKeyDurationMs - (millis() - shareKeyGeneratedAt);
        if (remain > shareKeyDurationMs) remain = 0;     // 溢出保护
        payload += String(remain / 1000);
    } else {
        payload += "0";
    }
    payload += "}";
...
void printStatus() {
    ...
    Serial.printf("Share key: %s\n", shareKey.length() > 0 ? shareKey.c_str() : "none");
    if (shareKey.length() > 0) {
        unsigned long remain = shareKeyDurationMs - (millis() - shareKeyGeneratedAt);
...
void printHelp() {
    ...
    Serial.println("  share:gen:<H>  share:revoke  (generate/revoke share key)");
    Serial.println("  auth:<key>     (authenticate via share key)");
    ...
    Serial.println("              AUTH:<key> / SHARE:GEN:<H> / SHARE:REVOKE");
    Serial.println("              (BIND blocked without PIN, share-key/pairing exempt)");
    Serial.println();
