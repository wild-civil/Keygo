 ...
 */
#define KEYGO_FW_VERSION   "3.36.3-fix28"  /* ★ v3.36.3-fix28 (2026-08-04): P14 修复 fix27 两个致命回归 —— ① 断连后加「快重连窗口」：普通 3s(50ms) / No-App 15s(20ms)，过期才经 STOP→RESTART 降至 5s 慢速（fix27 断连直降 5s→30s 超慢致"经常断一下就再也连不上，需重启设备"）；② 超慢延迟从 30s 拉到 5min（只有极长时间无连接才进 30s 省电）；③ 实测普通模式未连接最低 88µA（5s STOP→RESTART 生效）。前序 fix27: No-App 自举配对窗+TGAP 覆写修复(88µA 但连连不稳)。 */
