...
#define STATUS_JSON_MAX_LEN         224  // ★ Phase 2: 原 200 已含 14 字段；新增 m 模式字段，扩到 224 留余量
/* ★ v3.16-#26: 固件版本号 — 通过 FF02 Notify JSON 字段 "v" 上报
 *   App 可据此做兼容性检查，提示用户升级固件 */
#define KEYGO_FW_VERSION           "3.31.0-b1"     // ★ v3.31 方案B: FF02 新增 uc/lc/ucnt/lcnt/th + 事件驱动上报（解锁进度可见/配置回显验证）
...
void KeyGo_NotifyStatus(void)
{
    ...
    int n = snprintf(json, sizeof(json),
        "{\"c\":1,\"st\":\"%s\",\"r\":%d,\"f\":%d,\"d2\":\"%s\",\"cd\":%d,\"kr\":%d,\"al\":%d,\"bn\":%d,\"v\":\"%s\",\"uc\":%d,\"lc\":%d,\"ucnt\":%d,\"lcnt\":%d,\"th\":%d,\"m\":%d}",
