...
#define STATUS_JSON_MAX_LEN         272  // ★ v3.33: 新增 fwsec 能力字段，从 224 扩到 256；v3.36.1 再加 "t" 温度字段留余量
 ...
 *   ★ v3.36.0（2026-07-17）fwsec 1→2：授权体系 v1（per-phone 身份 + 每 owner RSSI 阈值跟随），
 *     详见 docs/授权体系v1_per-phone与RSSI阈值跟随.md。新固件须配新 App（旧 App 走 gk 兼容分支）。 */
#define KEYGO_FWSEC                 2
...
/* ★ LED 跟随脉冲：蓝 LED(PB14) 仅在 GPIO 控制脉冲 ON 时亮，脉冲结束灭（平时全灭省电）。
 *   不再有独立闪烁状态机（已删除 SBP_LED_*_BLINK_EVT 腾出事件位）。
 *   红 LED(PB15) 仅用于长按恢复出厂重大提示，走独立 GPIO 逻辑，不受影响。 */
...
static void KeyGo_FactoryReset_Poll(void)
{
    ...
    if (g_frState == 0) {
        if (pressed) {
            ...
            g_frFast      = 0;
            g_frLastPct   = 0;
            /* 独占红 LED 做反馈：蓝 LED 灭（平时本就灭，这里确保无残留脉冲亮） */
    ...
    if (g_frState == 1) {  // arming：按住累计中
        if (!pressed) {    // 提前松开 → 取消
            GPIOB_ResetBits(PIN_LED_BLUE_GPIO);   // 蓝 LED 灭(取消复位, 平时不亮)
 ...
 *   序列: ON(RIDE_HALF_TICKS≈350ms) → OFF(RIDE_GAP_TICKS≈200ms) → ON(RIDE_HALF_TICKS≈350ms) → OFF。
 *   比原 100/150ms 略慢，肉眼清晰识别为「快速按两下」而非常亮。由 KeyGo_RidePulseHandler
 *   按 g_rideStep 推进。仅 ebike 模式调用；car 模式由 HandleCommand 直接拒绝。
 * ───────────────────────────────────────────────────────────────── */
static uint8_t g_rideStep = 0;
static uint8_t g_rideExitStep = 0;   // ★ 骑行退出链式上锁状态: 0=空闲 1=解锁脉冲中(等 SBP_GPIO_PULSE_END_EVT) 2=解锁完成(等 SBP_RIDE_EXIT_LOCK_EVT 上锁)
...
/* ★ 骑行退出链式上锁：解锁脉冲结束后由 SBP_RIDE_EXIT_LOCK_EVT 触发，
 *   输出 LOCK 脉冲完成「退出骑行 → 上锁」。独立事件驱动，时序确定且兼容 HAL_SLEEP。
 *   原实现借状态机 125ms 轮询间隙上锁，已废弃（见 KeyGo_ProcessStateMachine 移除处）。 */
void KeyGo_RideExitLockHandler(void)
{
    if (g_rideExitStep != 2) return;   // 仅处理「解锁脉冲已结束」后的上锁
    g_rideExitStep = 0;
    g_keyState     = KSTATE_LOCKED;
    KeyGo_Lock();
    KeyGo_NotifyStatus();
    PRINT("[LOCK] ride-exit → lock done (event-driven)\n");
}
...
void KeyGo_GPIO_PulseEnd(void)
{
    ...
    g_pulsePinMask  = 0;
    g_actionActive  = 0;
    /* ★ 骑行退出链式上锁：本脉冲是「解锁退出骑行」的脉冲 → 延迟启动独立事件输出 LOCK
     *   （不再依赖状态机轮询，HAL_SLEEP 下更可靠；时序由 RIDE_EXIT_LOCK_DELAY_TICKS 决定） */
    if (g_rideExitStep == 1) {
        g_rideExitStep = 2;
        /* ★★★ 想调「结束骑行模式中间的间隔」(先解锁退出骑行 → 隔多久再上锁) 就改这里用到的宏：
         *   peripheral.h 里的 RIDE_EXIT_LOCK_DELAY_TICKS（当前 3200 tick ≈ 2s）。
         *   嫌长就调小（如 1600≈1s），嫌短就调大。改这一处即可，勿动此处调用。★★★ */
...
/* [LED_REMOVED] 后备箱/骑行 LED 闪烁 TMOS 回调已删除（v3.36.x）：
 *   LED 改为跟随 GPIO 脉冲（解锁/锁/后备箱脉冲期间亮、结束灭；骑行跟随双脉冲闪两下），
 *   不再需要独立闪烁事件，腾出的 0x0040/0x0200 供骑行退出链式上锁等新事件使用。 */
...
void KeyGo_ProcessStateMachine(void)
{
    ...
    /* ★ 骑行退出链式上锁已改独立事件 SBP_RIDE_EXIT_LOCK_EVT（见 KeyGo_GPIO_PulseEnd →
     *   KeyGo_RideExitLockHandler），不再依赖状态机轮询间隙，本处移除。 */
 ...
 *   adc_to_temperature_celsius() 按出厂校准(ROM)换算成摄氏度整数。
 *   节流采样（默认 5s）缓存，降低对 BLE 事件时序的影响；
 *   采样结果经 FF02 status JSON 的 "t" 字段（摄氏度整数）上报给 App。
 * ───────────────────────────────────────────────────────────────── */
#define TSENSE_SAMPLE_INTERVAL_MS   5000
static int16_t  s_cachedTempC = 0;
static uint32_t s_lastTempMs  = 0;
static uint8_t  s_tempInit    = 0;

int16_t KeyGo_ReadTemperatureC(void)
{
    uint32_t now = Peripheral_GetSystemMs();
    if (!s_tempInit || (now - s_lastTempMs) >= TSENSE_SAMPLE_INTERVAL_MS) {
        uint16_t raw = HAL_GetInterTempValue();
        s_cachedTempC = (int16_t)adc_to_temperature_celsius(raw);
        s_lastTempMs  = now;
        s_tempInit    = 1;
        PRINT("[TSENSE] raw=%u temp=%dC\n", raw, s_cachedTempC);
    }
    return s_cachedTempC;
}
...
void KeyGo_NotifyStatus(void)
{
    ...
    // ★ v3.36: 区间判定与状态机一致——优先用「当前已鉴权 owner」的阈值，否则全局配置（与 KeyGo_ProcessStateMachine 同源）。
    int16_t _ou = g_cfgUnlockThreshold, _ol = g_cfgLockThreshold;
    Bonding_GetActiveOwnerRssi(&_ou, &_ol);   /* 无 owner 身份时返回全局值（不变）*/
    uint8_t th = 0;
    if (g_filteredRSSI != RSSI_UNINITIALIZED_F) {
        if (g_filteredRSSI > _ou)      th = 1;
        else if (g_filteredRSSI < _ol) th = 2;
    }
    ...
    // ★ v3.36.1: TSENSE 内部温度遥测（节流采样，详见 KeyGo_ReadTemperatureC）
    ...
    int n = snprintf(json, sizeof(json),
        ...
        (int)_ou, (int)_ol,         // ★ v3.36: 当前生效的解锁/锁车阈值（owner 专属或全局，供 App 调试显示）
    ...
    if (n > 0 && n < (int)sizeof(json)) {
        ...
        if (noti.pValue) {
            ...
            bStatus_t notiSt = simpleProfile_Notify(peripheralConnList.connHandle, &noti);
            if (notiSt != SUCCESS) {
