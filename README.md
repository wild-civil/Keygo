...
| **蓝牙钥匙命名** | 每台设备可设易记自定义名（持久化 `ble_device_names`），控制页顶部只显示纯自定义名，连接页 / 扫描列表显示「自定义名 (KeyGo-XXXXXX)」组合格式 | 已落地（2026-07-23） |
| **已配对设备识别** | `knownDevices` 集合内设备打「已配对」徽章，区分历史连过的设备与陌生设备 | 已落地（2026-07-23） |
| **多设备：已知设备列表 + 默认置顶** | `knownDevices` 记录所有连过设备；断开后显示可滚动重连列表（默认置顶 + 最近连接倒序）；「默认」仅影响排序，不改自动重连目标 | 已落地（2026-07-23）；多设备 UI 仅代码审查，单设备未真机验证 |
...
- 「已知设备集合」当前 = 缓存的 `ble_device_id`（过渡期）。**非已知设备绝不会自动连接**，避免误连邻居家/陌生人的车钥匙（方案B 安全边界）。
  （注：此处「已知设备集合」指自动重连目标 `lastDeviceId` / `ble_device_id`；App 另维护一个 `knownDevices` **全量**已知集合用于「重连列表 UI / 默认设备 / 已配对徽章」，二者解耦，见下节「多设备」。）
...
> 📱 **App 扫描页「扫不到已配对设备」是否正常？** —— **正常，且已自动处理**。
> 无App模式下，已配对手机的 OS 会在设备进范围时自动加密重连（耳机式）→ 固件作为**单连接从机**在连接建立后**停止广播** → App 的 BLE 扫描自然扫不到。这是 BLE 单连接的物理事实，**不是 bug**。
> 为此 App 在扫描页进入时已做优化：**先调用系统 `getConnectedBluetoothDevices` 查询**，若命中本设备（按 MAC 匹配已知设备），**直接复用该 GATT 连接并跳过扫描**（无需手动重连，自动走订阅 FF02 + 读序列号 + AUTH）。仅当系统确实未占用连接时，才走普通 BLE 扫描。
> 判断口诀：固件串口显示 `Connected 1 - Int 18`（被占用、不广播）却扫不到 → 属正常，App 会自动复用；若串口显示 `Advertising..` 仍扫不到 → 才属异常（多为国产 ROM 扫描栈静默失败，见主控扫描冷却机制）。
...
### 蓝牙钥匙命名
- 每台设备可设易记自定义名（默认回退 `KeyGo-{MAC后6位}`），存 `ble_device_names`（按 MAC 索引，持久化）。
- 显示规则：控制页顶部车辆大卡**只显示纯自定义名**；连接页 / 扫描列表的「已连接 / 已配对」行显示「自定义名 (`KeyGo-XXXXXX`)」组合格式。
- 出厂名 `KeyGo-XXXXXX` 的 XXXXXX 取自广播名里的 MAC 字节（固件小端字节序，与手机蓝牙列表一致），非随机。

### 多设备：已知设备集合 + 默认设备
- **已知设备集合 `knownDevices`**（持久化 `ble_known_devices`）：任何一次「成功连接」都记录该设备 MAC + 最近连接时间；兼容旧 `ble_last_device_id`（首次连自动并入）。这是「所有连过设备」清单，与「自动重连目标 `lastDeviceId`」**解耦**——自动重连仍只认 `lastDeviceId`，行为不变。
- **「已知设备」重连框**：断开连接后，连接页 / 控制页显示已知设备列表，按 **默认设备置顶 + 最近连接倒序** 排列；单台 = 单卡（与旧版一致），≥2 台 = 可滚动列表；点列表项即重连该设备。
  - ⚠️ **出现时机**：该框仅在 `未连接` 且 `knownDevices` 非空时显示。已连设备 A 时框隐藏 → 多设备想从 A 切到 B 需先断开；首次安装从未连过时无框（符合预期）。
- **默认设备 `defaultDeviceId`**（持久化 `ble_default_device_id`）：在列表项上标「默认」即可置顶；仅影响「已知设备」框排序展示，不改自动重连目标。
- **已配对徽章**：在「已连接 / 已知设备」列表里，`knownDevices` 记录过的设备打「已配对」徽章，便于区分陌生设备与历史设备。
...
### 串口调试命令（UART Debug Commands，v3.36.3-fix4）

固件在 **DEBUG 构建**下支持串口（UART1）实时调试命令，用于观察无App模式 / per-phone 阈值 / RSSI 行为（波特率同固件配置，默认 115200；命令不区分大小写，回车结尾）。

> 开关为**位掩码分类**（`verbose` 为总闸），仅 DEBUG 下生效；非 DEBUG 构建日志被完全编译剔除、零开销。默认仅 `scan` / `rssi` 开，其余分类全静默。

| 命令 | 控制变量 / 位 | 默认 | 日志前缀 | 含义 |
|------|--------------|------|----------|------|
| `scan on\|off` | `g_scanLogEnabled` | 开 | — | 扫描请求日志（低频，默认开） |
| `rssi on\|off` | `g_rssiLogEnabled` | 开 | `[RSSI]` | **验证 per-phone 阈值是否真正生效的关键**：打印 `using owner threshold unlock=-45 lock=-55`，应与 `[BOND]` 行指纹识别结果一致 |
| `verbose on\|off` | `g_logMask` 总闸 | 关 | 全部 | 一键全开 / 全关所有分类诊断日志（= `obs\|raw\|state\|diag\|rssiset\|gap` 全部） |
| `obs on\|off` | `LOG_OBS` | 关 | `[OBS] rssi` | 周期 RSSI 观察（无App 模式 ≈1s 一次，刷屏头号源），看实时信号漂移 |
| `raw on\|off` | `LOG_RAW` | 关 | `[RAW]` | RAW 通知队列 enqueue/flush |
| `state on\|off` | `LOG_STATE` | 关 | `[STATE]` | 状态机「解锁/锁车阈值到达」「手动命令冷却结束」 |
| `diag on\|off` | `LOG_DIAG` | 关 | `[DIAG]` | 连接参数（LinkEst/ParamUpd）、owner 列表（双视图）、断连原因 |
| `rssiset on\|off` | `LOG_RSSISET` | 关 | `[RSSISET]` | RSSI 校准 enter/exit/owner 结果 |
| `gap on\|off` | `LOG_GAP` | 关 | `[GAP]` | adv/conn healthy 周期健康检查 |
| `help` | — | — | — | 列出所有命令 |

- **无参数 = 切换该开关**（如 `obs` 在开/关间切换）；`on` 强制开，`off` 强制关。
- **始终输出（不受任何开关影响）**：连接/断连及原因（`[OBS] CONNECTED`/`[OBS] DISCONNECTED`）、解锁/上锁/后备箱（`[KEY]`）、绑定成功/失败（`[BIND]`）、加密升降（`[OBS] LINK_ENCRYPTED`/`LINK_PLAIN`）、指纹识别结果（`[BOND] No-App reconnect: owner N identified ... (per-phone ON, unlock=.. lock=..)`）、看门狗 / 错误 / 警告类。
- **典型排查流程**：默认 `rssi` 已开，重连后看 `[RSSI] using owner threshold` 是否与 `[BOND]` 行 `unlock/lock` 一致 → 一致即 per-phone 阈值真生效；需看实时 RSSI 走势再 `obs on`（调试完 `obs off` 关掉以免刷屏）。
