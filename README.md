...
- 固件 `KEYGO_FW_VERSION`：`3.36.0`（与 App 版本号一致，无需重烧）。
- App 版本标注：`v3.36.0`（与固件一致；见 `stores/ble.js` 的 `APP_VERSION` 常量）。
...
| `0000FF02-...` | Read, Notify | 设备→App | 状态上报 | JSON：`{"c":1,"st":"LOCKED","r":-52,"f":-52,"b":85,"d2":"","cd":8000,"kr":15,"al":1,"bn":1,"v":"3.36.0","m":0,"uc":3,"lc":5,"ucnt":1,"lcnt":0,"th":1,"ou":-45,"ol":-65,"fwsec":2}` |
...
└── docs/                             # 设计文档 & 复盘（详见 docs/README.md 索引）
    ...
    ├── 03-复盘与问题分析/            # 版本总结、专题分析、实现状态台账、BLE 连接/扫描复盘
    ├── 自己看/                       # 离线打包、原生调试等实操记录（不构成设计决策）
